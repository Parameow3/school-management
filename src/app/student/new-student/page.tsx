...
const Page= ()=> {
  ...
  const handleBranchChange = (selectedBranchId: number | null) => {
    setFormData({
      ...formData,
      branch_id: selectedBranchId, // Allow null for "All branches"
    });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    Object.entries(formData).forEach(([key, value]) => {
        if (key === "image" && value) {
            ...
            value.forEach((v) => data.append(`${key}[]`, v)); // Append arrays properly
        } else if (value !== null) {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/academics/students/`, // Assuming we're creating a program
      ...
      router.push("/student/all-student"); // Redirect to program list or reset form
    } catch (error) {
        ...
        <section>
          ...
          <div className="grid lg:grid-cols-3 flex-col gap-8">
            ...
            <div>
              ...
              <Dropdown
                value={formData.branch_id ?? undefined} // Use undefined if branch is null
