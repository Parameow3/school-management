...
const getUserNames = (userIds: number[]): string => {
    ...
    useEffect(() => {
        ...
        const fetchStudentAndProgramData = async () => {
            try {
                // Fetch students data
                const studentResponse = await axios.get('http://127.0.0.1:8000/api/academics/student_trail/', {
    ...
    return (
        ...
        <span className="flex flex-row gap-2 text-[14px] lg:text-[15px]">
          Student | <Image src="/home.svg" width={15} height={15} alt="public" /> Student Trail 
        </span>
                    ...
                    {filteredStudents.length > 0 ? (
                        filteredStudents.map(student => (
                            <tr key={student.id}>
                                ...
                                <td className="border px-4 py-2 text-[14px]">{student.client}</td>
                                <td className="border px-4 py-2 text-[14px]">{student.phone}</td>
                                <td className="border px-4 py-2 text-[14px]">{student.number_student}</td>
                                <td className="border px-4 py-2 text-[14px]">{getProgramNames(student.programs)}</td>
                                <td className="border px-4 py-2 text-[14px]">{student.status_display}</td>
                                <td className="border px-4 py-2 text-[14px]">{getUserNames([student.assign_by])}</td>
                                <td className="border px-4 py-2 text-[14px]">{getUserNames(student.handle_by)}</td>
