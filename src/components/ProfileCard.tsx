...
interface ProfileCardProps {
  pic?: string;
  job?: string;
  onViewClick?: (viewPath: string | undefined) => void;
  onEditClick?: (path: string | undefined) => void;
  onDeleteClick?: () => void;
  editPath?: string;
  viewPath?: string;
  id?: number;
...
const ProfileCard: React.FC<ProfileCardProps> = ({
  ...
  // Fallback if `pic` is undefined or null
  const validPicSrc = pic ?? "";
      ...
      <Card className="lg:w-[243px] lg:h-[244px] w-[139px] h-[173px] shadow-sm bg-white rounded-[9px] transition-transform transform hover:scale-105 hover:shadow-lg hover:bg-gray-100">
        <CardBody className="flex flex-col items-center p-4 overflow-hidden">
          <Image
              ...
              src={validPicSrc}
              alt={first_name || "Unnamed"}  // Ensure alt is always a string
              unoptimized
          />
          <b className="flex items-center lg:text-lg text-[12px] lg:mt-4 text-center">
            {first_name}
          </b>
          <p className="text-default-500 lg:text-[18px] text-[11px] text-center">
            {job}
          </p>
        </CardBody>
